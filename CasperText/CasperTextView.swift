...
public class CasperTextView: UITextView {
    ...
    fileprivate func resetBottomBarFrame() {
        let y = min(max(contentSize.height + 4, (placeholderLabel?.frame.maxY ?? 0) + 4), bounds.height - 2)
        bottomBar?.frame = CGRect(x: 0, y: bounds.origin.y + y, width: bounds.width, height: 2)
    }
    ...
    public var isFloatingLabelDisplayed: Bool {
        return (self.floatingLabel?.alpha ?? 0) > 0
    }
